...
@section('content')
    <div class="panel">
        ...
        <p style="color: var(--muted); margin-top:6px;">Register a service, then assign the stylists who are allowed to perform it.</p>
        ...
        <form method="POST" action="{{ route('services.store') }}" style="margin-top:14px; display:grid; gap:14px;">
            ...
            <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(220px,1fr)); gap:14px;">
                <label style="display:flex; flex-direction:column; gap:6px; font-weight:600;">
                    Service name
                    <input name="name" type="text" value="{{ old('name') }}" placeholder="Wash and blow dry" required style="padding:12px;border:1px solid #e5e7eb;border-radius:10px;">
                </label>
                <label style="display:flex; flex-direction:column; gap:6px; font-weight:600;">
                    Category
                    ...
                    @if($categories->isEmpty())
                        <small style="color:#b45309; font-weight:500;">
                            No service categories exist for this business yet.
                            <a href="{{ route('service-categories.create') }}" style="color:#0369a1; font-weight:600;">Add a service category</a>
                        </small>
                    @endif
                </label>
                <label style="display:flex; flex-direction:column; gap:6px; font-weight:600;">
            ...
            <div>
                <div style="font-weight:700; margin-bottom:8px;">Assign stylists to this service</div>
                @if($workers->isEmpty())
                    <div style="color:#b45309; font-size:14px;">
                        No stylists are registered for the active branch yet. Use the Services page to add them first.
                    </div>
                @else
